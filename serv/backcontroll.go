...
func DoDeleteRes(c *gin.Context) bool {
	...
	if delerr != nil {
		log.Err(delerr).Msg("删除失败:" + inputTime)
		return false
	}
	return true
}
...
func SetAnnounce(c *gin.Context) bool {
	...
	insertMap["text"] = c.PostForm("announceText")
	insertMap["updatetime"] = time.Now().Format("2006-01-02 15:04:05")
	annCollection.DeleteMany(context.TODO(), bson.M{})
	_, setAnnErr := annCollection.InsertOne(context.TODO(), insertMap)
	if setAnnErr != nil {
		log.Err(setAnnErr).Msg("设置公告失败：" + c.PostForm("announceText"))
		return false
	}
	return true
}
...
func GetAnnounce(c *gin.Context) bool {
	...
	results, err := annCollection.Find(context.TODO(), bson.M{})
	defer func() {
		results.Close(context.TODO())
	}()
	if err != nil {
		...
		log.Err(err).Msg("查询全部公告失败")
		return false
	}
	for results.Next(context.TODO()) {
		annMap := make(map[string]string)
		if err = results.Decode(&annMap); err != nil {
			log.Err(err).Msg("查询全部公告失败")
			c.JSON(http.StatusOK, gin.H{
				"msg": "查询全部公告失败",
			})
...
func DelAnn(c *gin.Context) bool {
	log := logutil.GetLog()
	annCollection := dboption.GetAnnCollection()
	_, err := annCollection.DeleteMany(context.TODO(), bson.M{})
	if err != nil {
		log.Err(err).Msg("删除公告失败")
		c.JSON(http.StatusOK, gin.H{
			"msg": "删除公告失败",
		})
		return false
